...
/**
 * SimulationMaterial: GPU-based particle physics using Perlin noise
 * Creates smooth, organic waving motion like a sea of dots
 */
    ...
    super({
      ...
      fragmentShader: /* glsl */ `
        ...
        void main() {
          ...
          // --- Morphing Grid Logic ---
          // By adding a time-based offset to the noise input, we make the
          // noise field "flow" through the grid, creating a morphing effect.
          float morphSpeed = 0.2;
          vec3 timeOffset = vec3(
            continuousTime * morphSpeed,
            continuousTime * morphSpeed * 0.8, // Slightly different speed for Y
            0.0 // The Z-component of noise is already handled by the periodicNoise function
          );

          vec3 noiseInput = (originalPos * uNoiseScale) + timeOffset;
